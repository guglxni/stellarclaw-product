	...
	go func() {
		...
		for {
			...
			var gu struct {
				...
				Result []struct {
					...
					Message  *struct {
						...
						// Telegram document attachment (PDF, CSV, ZIP, etc.)
						Document *struct {
						...
						// Telegram photo attachment (array of sizes, largest last)
						Photo []struct {
							FileID   string `json:"file_id"`
							FileSize int64  `json:"file_size"`
			...
			for _, upd := range gu.Result {
				...
				// Build message content — inject file/photo metadata into the text
				// so the LLM knows a file was sent and can extract file_id for MCP tools.
				// This mirrors how Discord's handler appends [attachment: url] to messages.
				text := m.Text
				if text == "" && m.Caption != "" {
					text = m.Caption
				}
				if m.Document != nil {
					fileInfo := fmt.Sprintf("[File received: %s (%s, %d bytes, file_id=%s)]",
						m.Document.FileName, m.Document.MimeType, m.Document.FileSize, m.Document.FileID)
					if text != "" {
						text = fileInfo + "\n" + text
					} else {
						text = fileInfo
				...
				if len(m.Photo) > 0 {
					...
					photoInfo := fmt.Sprintf("[Photo received: file_id=%s]", largest.FileID)
					if text != "" {
						text = photoInfo + "\n" + text
					} else {
						text = photoInfo
