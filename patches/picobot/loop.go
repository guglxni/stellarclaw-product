...
type AgentLoop struct {
	...
	showToolActivity bool // when false, suppresses "Running:" / "done" messages to users
}
			...
			for iteration < a.maxIterations {
				...
				if resp.HasToolCalls {
					...
					for _, tc := range resp.ToolCalls {
						// Only send tool activity notifications if enabled in config
						if a.showToolActivity {
